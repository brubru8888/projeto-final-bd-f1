"""Acesso a dados do piloto."""


class Piloto_dao:
    """Consultas do perfil piloto."""

    def __init__(self, db_pool):
        self._db_pool = db_pool

    def get_years(self, driver_id):
        """Retorna o primeiro e último ano do piloto."""
        conn = None
        try:
    ...
    def get_piloto_info(self, driver_id):
        """Busca nome e escuderia mais recente."""
        conn = None
        try:
            conn = self._db_pool.getconn()
            cursor = conn.cursor()
    ...
    def get_r6_pontos_ano(self, driver_id):
        """Lista pontos por corrida e ano."""
        conn = None
        try:
            conn = self._db_pool.getconn()
            cursor = conn.cursor()
    ...
    def get_r7_status_piloto(self, driver_id):
        """Lista a distribuição de status do piloto."""
        conn = None
        try:
            conn = self._db_pool.getconn()
            cursor = conn.cursor()
